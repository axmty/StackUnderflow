...
module HttpHandlers =
  ...
  let activityTypes = [ "asked"; "answered"; "modified" ]
    ...
    new Collections.Generic.List<PostSummaryDto>(
      [ for i in 1..100 ->
          { VoteCount = i - 1
            ...
            HasAcceptedAnswer = List.item (i % 2) [ true; false ]
            Username = "Alexandre Metgy"
            UserReputationScore = rand.Next(1000)
            LastActivityRelativeTime = "2 mins ago"
            LastActivityType = activityTypes[rand.Next(activityTypes.Length)] } ]
    )
      ...
      task {
        ...
        let newQuestion =
          { VoteCount = 0
            ...
            UserReputationScore = rand.Next(1000)
            LastActivityRelativeTime = "1 min ago"
            LastActivityType = "asked" }
